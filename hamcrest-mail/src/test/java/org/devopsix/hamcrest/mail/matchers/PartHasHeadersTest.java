...
public class PartHasHeadersTest {
  ...
  public void shouldNotMatchWhenHeadersCannotBeExtracted() throws Exception {
    ...
    when(message.getHeader(eq("Received"))).thenThrow(
        new MessagingException("error decoding headers"));
  ...
  public void shouldMatchWhenTwoHeadersArePresent() {
    Message message = messageWithTwoHeaders("Received",
        "by foo (MTA) id abc123; Sun, 1 Dec 2019 12:27:39 +0100 (CET)",
  ...
  public void shouldMatchWhenHeadersWithGermanUmlautsInUtf8AndIso88591IsPresent() throws Exception {
    Message message = messageWithTwoHeaders("Some-Header", encodeText("ÄÖÜ", "UTF-8", null),
        encodeText("äüöß", "ISO-8859-1", null));
    PartHasHeaders matcher =
        new PartHasHeaders("Some-Header", hasItems(equalTo("ÄÖÜ"), equalTo("äüöß")));
    assertThat(message, matcher);
  }
