...
public class MessageHasRecipientsTest {
  ...
  public void shouldNotMatchWhenRecipientsOfCertainTypeCannotBeExtracted() throws Exception {
    Message message = mock(Message.class);
    when(message.getRecipients(any(RecipientType.class))).thenThrow(
        new MessagingException("error decoding recipients"));
  ...
  public void shouldMatchRecipients() throws Exception {
    Message message = mock(Message.class);
    when(message.getAllRecipients()).thenReturn(
        new Address[] {new InternetAddress("joe.average@example.com")});
  ...
  public void shouldMatchRecipientsOfCertainType() throws Exception {
    Message message = mock(Message.class);
    when(message.getRecipients(any(RecipientType.class))).thenReturn(
        new Address[] {new InternetAddress("joe.average@example.com")});
