...
public class PartHasDateHeadersTest {
    ...
    public void shouldNotMatchWhenHeaderCannotBeExtracted() throws Exception {
        ...
        when(message.getHeader(eq("X-Custom"))).thenThrow(new MessagingException("error decoding header"));
        PartHasDateHeaders matcher = new PartHasDateHeaders("X-Custom", emptyIterableOf(OffsetDateTime.class));
